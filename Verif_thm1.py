...
def Plot_comparaison_distributions(n_values=None, methode='parallele'):
    """
    Compare les trois distributions sur un même graphique.
    ...
    distributions = [
        ('exponentielle', {'scale': 0.2}, 'Exponentielle (λ=0.2)', 'bo-'),
        ('uniforme', {'a': 0, 'b': 1}, 'Uniforme [0,1]', 'rs-'),
        ('beta', {'alpha': 1, 'beta': 5}, 'Bêta (α=1, β=5)', 'g^-')
    ]
    
    plt.figure(figsize=(12, 8))
    
    for dist_name, params, label, style in distributions:
        print(f"\nCalcul pour {label}...")
        proba_valeurs = []
        
        for n in n_values:
            print(f"  n={n}...")
            proba = Proba_optimisee(n, distribution=dist_name, nb_echantillons=15, 
                                   nb_simulations=600, methode=methode, **params)
            proba_valeurs.append(proba)
        
        proba_valeurs = np.array(proba_valeurs)
        plt.plot(n_values, proba_valeurs, style, linewidth=2, markersize=8, label=label)
    ...
    plt.xlabel("Nombre de joueurs n")
    plt.ylabel("Probabilité que le meilleur gagne")
    plt.title("Comparaison des distributions - Probabilité de victoire du meilleur joueur")
    plt.legend()
    plt.grid(True, which="both", ls="--", alpha=0.3)
    plt.ylim(0, 1)
    plt.tight_layout()
...
## EXEMPLES D'USAGE
"""
# Distribution exponentielle (comme avant)
proba_exp = Proba_optimisee(20, 'exponentielle', scale=0.2)

# Distribution uniforme sur [0,1]  
proba_unif = Proba_optimisee(20, 'uniforme', a=0, b=1)

# Distribution bêta(1,5)
proba_beta = Proba_optimisee(20, 'beta', alpha=1, beta=5)
...
# Comparaison des trois distributions
Plot_comparaison_distributions()
"""